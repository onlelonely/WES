...
subprocess.call("/usr/local/bin/bwa mem -t 12 -M -R '@RG\tID:bwa\tSM:1485\tPL:illumina' {index} {F} {R} | /usr/local/samtools/bin/samtools sort -O BAM -@ 12 -o {out}".format(index=args.references, F=args.read_F, R=args.read_R, out=args.out),shell=True, executable='/bin/bash')
